...
class BNTUContentParserService:
    ...
    def sync_literature_bootstrap(self):
        items = asyncio.run(self._collect_literature_bootstrap())
        if not items:
            return 0

        existing_source_ids = set(
            LiteratureItem.objects.filter(source_id__in=[item["source_id"] for item in items]).values_list("source_id", flat=True)
        )
        rows = [self._build_literature_item(item) for item in items if item["source_id"] not in existing_source_ids]
        if rows:
            LiteratureItem.objects.bulk_create(rows, batch_size=300)
        return len(rows)
    ...
    def sync_literature_incremental(self):
        ...
        items = asyncio.run(self._collect_literature_incremental(existing_source_ids))
        if not items:
            return 0

        rows = [self._build_literature_item(item) for item in items]
        if rows:
            LiteratureItem.objects.bulk_create(rows, batch_size=200)
        return len(rows)
        ...
        async with aiohttp.ClientSession(connector=connector, timeout=timeout, headers=DEFAULT_HEADERS) as session:
            while True:
                ...
                for index in range(0, len(candidates), 20):
                    batch = candidates[index:index + 20]
                    results = await asyncio.gather(
                        *[self._fetch_literature_payload(session, item) for item in batch],
                        return_exceptions=True,
                    )
            ...
            while True:
                ...
                for item in page_items:
                    ...
                    if item_id in existing_source_ids:
                        return collected
                    payload = await self._fetch_literature_payload(session, item)
                    if not payload:
                        continue
    ...
    async def _fetch_news_page(self, session, page):
        try:
            async with session.get(f"{NEWS_BASE_URL}/times?page={page}", ssl=False) as response:
    ...
    async def _fetch_literature_page(self, session, offset):
        try:
    ...
    async def _fetch_literature_metadata(self, session, item_id):
        try:
            async with session.get(f"{LITERATURE_REST_BASE_URL}/items/{item_id}/metadata", ssl=False) as response:
                if response.status != 200:
                    return []
                return await response.json(content_type=None)
        except (aiohttp.ClientError, ValueError):
    ...
    async def _fetch_literature_payload(self, session, item):
        item_id = item.get("id")
        if not item_id:
            return None

        metadata, expanded = await asyncio.gather(
            self._fetch_literature_metadata(session, item_id),
            self._fetch_literature_expanded(session, item_id),
        )
        return self._build_literature_payload(item, metadata, expanded)
    ...
    def _build_literature_payload(self, item, metadata, expanded):
        meta_dict = self._metadata_to_dict(metadata)
        title = (
            meta_dict.get("dc.title")
            or item.get("name")
            or ""
        )
        authors = ", ".join(str(value) for value in self._ensure_list(meta_dict.get("dc.contributor.author")) if value)
        publishing_date = str(meta_dict.get("dc.date.issued") or "")
        description = str(meta_dict.get("dc.description.abstract") or meta_dict.get("dc.description") or "")
        handle = str(item.get("handle") or "")
        uri = str(meta_dict.get("dc.identifier.uri") or "")
        literature_link = uri or (f"https://rep.bntu.by/handle/{quote(handle)}" if handle else "")

        parent_collection = expanded.get("parentCollection") or {}
        parent_communities = expanded.get("parentCommunityList") or []
        category = (
            parent_collection.get("name")
            or meta_dict.get("dc.subject")
            or meta_dict.get("dc.type")
            or ""
        )
        faculty = ""
        if parent_communities:
            faculty = str(parent_communities[0].get("name") or "")

        bitstreams = expanded.get("bitstreams") or []
        primary_bitstream = next(
            (
                bitstream for bitstream in bitstreams
                if not bitstream.get("bundleName") or bitstream.get("bundleName") == "ORIGINAL"
            ),
            bitstreams[0] if bitstreams else {},
        )
        download_link = self._normalize_rest_link(
            primary_bitstream.get("retrieveLink")
            or primary_bitstream.get("link")
            or ""
        )
        download_size = str(
            primary_bitstream.get("sizeBytes")
            or primary_bitstream.get("size")
            or ""
        )
        ...
        thumbnail = next(
            (
                bitstream for bitstream in bitstreams
                if str(bitstream.get("bundleName") or "").upper() == "THUMBNAIL"
            ),
            None,
        )
        if thumbnail:
            image_url = self._normalize_rest_link(
                thumbnail.get("retrieveLink") or thumbnail.get("link") or ""
            )
        ...
        return {
            ...
            "download_link": download_link or literature_link,
        }
