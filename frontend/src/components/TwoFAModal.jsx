...
import { X, Shield, Smartphone, RefreshCw } from 'lucide-react';
...
const TwoFAModal = ({ isOpen, onClose, onSuccess, darkMode, remainingTime = 300 }) => {
  ...
  const handleSubmit = async (fullCode) => {
    ...
    try {
      ...
      console.error('ERROR in handleSubmit:', requestError);
      setError(`Ошибка соединения с сервером: ${requestError.message}`);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="modal-backdrop fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-[100] p-4">
      <div className={`modal-panel w-full max-w-md ${darkMode ? 'bg-slate-900' : 'bg-white'} rounded-2xl shadow-2xl border ${darkMode ? 'border-slate-700' : 'border-slate-200'}`}>
